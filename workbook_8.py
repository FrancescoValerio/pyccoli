...
def make_the_file():
    ...
    volume = data['<VOL>']
    p_vol = [ (new / old) -1 for new, old in zip(volume[1:], volume)]
    del data['<TIME>']
    data_p = row_diff(data)
    del data_p['<DATE>']
    data_k = kmean_columns(data_p,5)
    data = data.iloc[1:,:]
    for column in data:
        if column == '<DATE>':
            continue
        data_k[f'og{column}'] = data[column]
    data_k['<DATE>'] = date
    
    data_k.to_excel('bitcoin2016.xlsx')

    export_to_pyccoli(filter_columns(data_k,'LB'), 'bitcoin2016')
